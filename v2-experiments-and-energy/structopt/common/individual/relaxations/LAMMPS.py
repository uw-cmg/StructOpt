...
class LAMMPS(object):
    ...
    @parallel
    def relax(self, individual, gparams = gparameters):
        """Relax an individual.

        Here we pass gparameters to LAMMPS.relax because when using mp.Pool, 
        the gparameters do not update, so the gparameters.generation always equal to 0.

        Args:
            individual (Individual): the individual to relax
        """
        # print('\n==========================')
        # print('LMP generation {}, individual {}'.format(gparams.generation, individual.id))
        # print('==========================\n')

        calcdir = os.path.join(self.output_dir, 'relaxation/LAMMPS/generation{}/individual{}'.format(gparams.generation, individual.id))
        rank = gparams.mpi.rank
        ...
        try:
            ...
            trj_file = os.path.join(gparams.logging.path, "modelfiles", "individual{}.trj".format(individual.id))
        ...
        if 'repair' in self.parameters and self.parameters['repair']:
            E = self.repair(individual, gparams.generation)
            if E is not None:
                individual.LAMMPS = E

        # print('===========================')
        # print('get energy: ', individual.LAMMPS)
        # print('===========================')
        return
